...
| # | Item | Refs | Effort |
|---|---|---|---|
| B1 | 🟡 **Reversible compression + retrieval handles** — per-session content-addressed `ContentStore` plus model-facing `expand_content(handle)` continuation in both non-streaming and streaming paths; bounded retrieval, eviction, and continuation tests pass. Config-gated OFF (`reversible_compression_enabled`) pending backend-validated streaming/retrieval benchmarks and backend-refusal coverage. | §4.1.2 | **[L]** |
