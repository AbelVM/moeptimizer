...
| # | Item | Refs | Effort |
|---|---|---|---|
| A1 | 🟡 **Adaptive budget — full policy + cache-break amortization trigger.** Add task-complexity / code-density / codebase-size signals to the ceiling; fold only when `marginal_per_turn_prefill × remaining_turns > re_prefill_cost`. **A1-lite is implemented:** `fold_window_fraction` default `0.25` gives zero breaks in the local persistent dry-run; the full horizon/complexity policy and backend validation remain pending. | §3.1, §4.2.2, §4.7.2 | **[L]** |
...
| A3 | ⬜ **Compaction-geometry redesign — the safe window-bound strategy (deferred).** The phaseA benchmark (A4 + A1-lite) has **zero cliffs across all 30 turns** (per-turn cache grows monotonically 0 → 24,254): below ~25% window utilization A1-lite fires **no folds**, so there are no fold-turn breaks at low utilization (the common case). A3's premise only holds for **window-bound** sessions (context climbing past ~25% of the window), which the opencode scenario (4%) never reaches. For window-bound sessions the proxy *must* fold to stay under the window; A3 would relocate the rolling summary so a fold does not shift the live zone, preserving the prefix cache across folds. Native `--context-shift` (F3) is **not** a safe alternative (rejected — see F3), so A3 remains the only cache-preserving option for window-bound sessions. **Defer:** complex ([L]) and only matters for window-bound sessions; until then A1-lite folding (with cache breaks on fold turns) is the window-bound fallback. | break attribution | **[L]** ⚠ cache |
...
| # | Item | Refs | Effort |
|---|---|---|---|
| B1 | 🟡 **Reversible compression + retrieval handles** — per-session content-addressed `ContentStore` plus model-facing `expand_content(handle)` continuation in both non-streaming and streaming paths; bounded retrieval, eviction, continuation, and backend-refusal tests pass (the refusal test verifies the model is offered `expand_content` yet returns plain content on a single backend call — the loop exits, no hang). Config-gated OFF (`reversible_compression_enabled`) pending backend-validated streaming/retrieval benchmarks. | §4.1.2 | **[L]** |
...
| D2 | ✅ **cache → TTFT correlation metric** — `cache_ttft` summary section: per-turn `fresh_prefill_tokens` stats + `fresh_prefill_vs_ttft_correlation` (Pearson) (commit `64a87e0`). The dashboard "Fresh prefill by turn" line chart + boxplot already render the series, and the dashboard now annotates that chart with the proxy fresh-prefill↔TTFT Pearson r (strong/moderate/weak) so the cache→TTFT mechanism is readable without digging into the JSON. | §4.12.3 | **[S]** |
...
| D4 | ✅ **Make degradation visible** — cumulative per-stage failure counts are surfaced in session debug and rolled up process-wide in `/v1/metrics`; regression-tested. `_record_degradation` now takes a structured `reason` code, and the code-slicing fail-open path emits `parser_unavailable:lang=<id>:size=<chars>:failed_open` (commits `a2e2bd9` + reason-fields follow-up; REVIEW_luna P1 — language, input size, and failed-open-vs-changed). Rolling the same structured reason codes out to the *other* optional stages is the remaining increment. | §4.11.3, §4.8.5 | **[M]** |
...
| # | Item | Refs | Effort |
|---|---|---|---|
| E1 | 🟡 **Decompose the god object** — `BudgetGovernor` was already out; now `PrefixLayout` moved to `prefix_layout.py` and four method clusters extracted verbatim as mixins (`thinking_ops.py`, `anchor_ops.py`, `diagnostics_ops.py`, `code_opt_ops.py`), taking `optimizer.py` **3737 → 2921 lines** with zero behavior change (545 tests + dry-run gate green; methods keep their names so diagnostic scripts still work). **Remaining:** the full `< ~1500` target needs splitting the 881-line `_optimize_messages_locked` pipeline (heavy local-state threading, ~32 hardcoded `# Step N` blocks, no `StageRunner`) — deferred as high-risk. | §4.2.3 | **[L]** |
| E2 | 🟡 **Typed summary region** replacing the scattered content-marker guards. **Scoped dedup done + tested:** the rolling-summary detector lived in three places — `_is_summary_block` was defined byte-identically in `optimizer.py` and `token_aware_truncator.py`, and a *third, divergent* raw guard in `compactor.py` checked only the `_summary_id`/`_rolling_summary` keys and missed the content marker (so a stripped summary could be front-evicted). All three now delegate to one shared `hierarchical_summarizer.is_summary_block`; regression tests cover the shared detector and the compactor's now-protected content-marker-only summary; dry-run stays at 0 breaks. **Deferred:** a full *typed* region object — several markers (`_DEDUP_MARKER`, `_ERROR_AWARE_MARKER`, `_TRUNC_MARKER`) are idempotency sentinels embedded in message *text* and cannot become a typed field, and the content-marker fallback must survive the strip/serialize round-trip (turn-11 cliff). | §4.7.3 | **[M]** |
...
| # | Item | Refs | Effort |
|---|---|---|---|
| F1 | 🟡 **ChatML / tool-schema / code-fence byte-stability hardening** (MTP draft acceptance). **Code-fence hardening done + tested:** injected volatile context runs through a deterministic `_balance_code_fences` so a dangling ```` ``` ```` (which `_placeholder_code_blocks` leaves, since it only strips balanced pairs) is closed within the trailing volatile turn and cannot bleed "code" into subsequent content; dry-run gate stays at 0 breaks. Tool-schema stability is already handled (client schema pinned, `expand_content` appended after) and injected turns are well-formed ChatML (string content, `_*` keys scrubbed at the boundary). **Remaining:** append-only validation on a genuinely window-bound session — blocked, like the rest of the release, on backend evidence. | §4.6.2 | **[M]** ⚠ cache |
...
| F3 | ❌ **Append-only + native `--context-shift` — REJECTED.** Letting llama.cpp's native `--context-shift` bound the window (shifting the KV cache when it fills) is **risky for MoE chat assistants on hardware-limited backends**: shifting the KV cache mid-conversation can cause unpredictable hardware stutter, severe model degradation, and broken chat logic. The proxy must NOT rely on native context-shift. Window-bound sessions are instead handled by A1-lite folding (cache breaks on fold turns) and — if preserving cache across folds becomes a priority — A3 (cache-safe folding). At *low* utilization A1-lite already behaves append-only (phaseA: zero cliffs), so nothing is lost by rejecting context-shift for the common case. | §4.3.1 | **[L]** ⚠ cache |
...
remaining A items are deferred (A2 maintainability; A3 window-bound only, see below).
**Window-bound strategy:** native `--context-shift` (F3) is **rejected** — risky for MoE
chat assistants on hardware-limited backends (hardware stutter, model degradation,
broken chat logic). So for window-bound sessions the proxy folds (A1-lite, cache breaks
on fold turns), and **A3 (cache-safe folding) remains the only cache-preserving option**
— deferred ([L]) until window-bound performance matters. B1 (reversible handles +
`expand_content` tool) is done but config-gated OFF pending a backend-validated
benchmark. C / D / E are largely independent and can interleave. Every ⚠-cache item must
be followed by the dry-run gate and a benchmark round.
...
| `reversible_compression_enabled` (B1) | `false` | 🟡 **stay OFF pending evidence** | The model-facing `expand_content(handle)` continuation is wired and covered by streaming/non-streaming tests, but backend refusal behavior and repeated live retrieval benchmarks are still unavailable. |
...
- **(A) Append-only + backend-managed eviction.** The proxy does only *lossless*
  ...
  history, and lets the backend bound the window. Maximum cache reuse; context fills
  the window (not "lean"), but on a 262K window that is fine and TTFT-optimal because
  every turn is a pure tail append. **Caveat:** the *low-utilization* half of this is
  achieved (A1-lite: no folds below ~25% window → append-only, phaseA zero cliffs), but
  the originally-proposed window-bound mechanism — llama.cpp `--context-shift` / native
  eviction — is **rejected for this deployment** (risky for MoE chat assistants on
  hardware-limited backends: hardware stutter, model degradation, broken chat logic).
  Window-bound sessions therefore fold (A1-lite / A3), not native context-shift.
...
window-bound sessions**, with **one** size governor instead of six. **Status:** the
low-utilization append-only of (A) is done (A1-lite → phaseA zero cliffs); the
window-bound mechanism is folding (B/A3), **not** native `--context-shift` (rejected
for this deployment — see option A caveat). The "one size governor" consolidation is
A2 (deferred).
...
1. 🟡 **Append-only at low utilization — achieved via A1-lite; native `--context-shift`
   REJECTED.** The append-only ideal (every turn a pure tail append → near-100 % reuse)
   is realized at *low* utilization by A1-lite (`fold_window_fraction=0.25`: no folds
   below ~25% of the window → phaseA zero cliffs). The originally-proposed mechanism for
   the window-bound case — letting `llama.cpp --context-shift` bound the window — is
   **rejected**: shifting the KV cache mid-conversation is risky for MoE chat assistants
   on hardware-limited backends (unpredictable hardware stutter, severe model
   degradation, broken chat logic). Window-bound sessions instead fold (A1-lite, cache
   breaks on fold turns) or, if cache preservation across folds matters, A3 (cache-safe
   folding, deferred). → Phase 2 (low-utilization append-only done; window-bound via A3).
...
5. ✅ **MED — silently swallowed exceptions.** Almost every stage is
   `try/except Exception → logger.warning` and uvicorn runs at `log_level="warning"`;
   a stage that consistently fails (e.g. tree-sitter parser missing) degraded quality
   with no test-visible signal. **Mostly fixed (D4):** `_record_degradation` now also
   increments a **cumulative per-stage counter** (`degradation_counts`, never resets)
   surfaced in `/v1/agent/sessions/{id}/debug`, with a regression test — so a
   consistently-failing stage accumulates and is visible/testable (this is exactly what
   would have caught the silent tree-sitter API bug). **Remaining:** a process-wide
   roll-up into `/v1/metrics` (the counts are per-session today). → Phase 4.
...
2. 🟡 **Append-only default for cache-stable mode** (§4.3.1): lossless compression of
   new content only. **Low-utilization append-only is done** (A1-lite → phaseA zero
   cliffs). Native `--context-shift` to bound the window is **rejected** for this
   deployment (risky for MoE on hardware-limited backends); genuinely window-bound
   sessions fold (A1-lite) or use cache-safe folding (A3, deferred).
