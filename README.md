...
| `MOEPT_AGENTIC__ADAPTIVE_BUDGET_ENABLED` | `true` | Replace the fixed optimized-context cap with an **adaptive, growing ceiling** (review §4.2.2). The budget starts at the static base and grows with the conversation horizon, capped at `adaptive_window_fraction` of the live window, and never shrinks below the current optimized size — so the immutable zones always fit (`reserved < budget`), the eviction governors stop fighting an unreachable target, and the rolling-summary fold fires far less often (higher prefix-cache reuse, more verbatim context). Set `false` to restore the legacy fixed cap. |
| `MOEPT_AGENTIC__ADAPTIVE_HORIZON_GROWTH_TOKENS` | `800` | Tokens added to the adaptive budget per conversation turn (the horizon term). A longer session carries a proportionally larger cached working set. |
| `MOEPT_AGENTIC__ADAPTIVE_WINDOW_FRACTION` | `0.15` | Ceiling on the adaptive budget as a fraction of the live backend window. Lets a long session grow toward ~15% of the window before the fold engages, instead of over-compacting against an idle window. |
| `MOEPT_AGENTIC__ADAPTIVE_CODE_DENSITY_FACTOR` | `0.25` | Task-complexity term of the adaptive budget (review §3.1): the budget grows by this fraction of the code tokens in the current request, so a code-heavy turn (refactor/debug with large file reads) keeps more verbatim code and folds less often (better prefix-cache reuse), while a prose-only turn keeps a tighter budget. `0` disables the term. |
...
| `MOEPT_AGENTIC__CODE_SLICING_ENABLED` | `false` | **Query-aware syntactic code slicing** (review §4.5.3 / Forward plan B4). For a message containing fenced code, keep the imports header plus the top-level definitions named in the session's original request, collapsing the rest (`slice_code_to_query`, tree-sitter). Strictly fail-open + never-expands + idempotent ⇒ cache-stable. OFF by default: it is context-dependent and should be benchmarked before enabling. |
...
| `MOEPT_AGENTIC__VOLATILE_QUALITY_ANCHOR_ENABLED` | `true` | Append the quality anchor (original request + accumulated constraints) to the trailing volatile turn on over-budget turns. The anchor is a redundant 3rd copy of the original request (also in the frozen prefix + rolling-summary head) and its stability is wasted because the RAG context in the same turn churns every turn (review §4.2.6), costing ~900 chars/over-budget-turn. Set `false` to drop it once benchmark quality confirms no regression. |
...
| `MOEPT_V050__FOLD_MARGIN_TURNS` | `0` | DRIFT fold margin: the live zone may drift this many turns past the keep window before a batch fold pulls it back. `0` = use the keep window. Larger ⇒ rarer folds (better prefix-cache reuse) but a larger context between folds. Only used when `FOLD_WINDOW_FRACTION` is `0`. |
| `MOEPT_V050__FOLD_WINDOW_FRACTION` | `0.25` | **Space-based folding** (compaction-geometry redesign, review §4.7 / Forward plan A1-lite). When `> 0`, the turn-count DRIFT trigger is disabled and the rolling summary folds only once the context exceeds this fraction of the live backend window — so on a huge window the conversation is near append-only and cache-breaking folds become rare/absent (the budget ceiling rises to match so the budget gates don't front-evict first). At low utilization append-and-reuse beats compact-and-break for TTFT. Default `0.25` = zero prefix breaks in the opencode/30 dry-run at ~5% of a 262K window. `0` = keep turn-count folding. |
