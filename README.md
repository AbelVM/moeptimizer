...
| `MOEPT_V050__FOLD_WINDOW_FRACTION` | `0.0` | **Space-based folding** (compaction-geometry redesign, review §4.7). When `> 0`, the turn-count DRIFT trigger is disabled and the rolling summary folds only once the context exceeds this fraction of the live backend window — so on a huge window the conversation is near append-only and cache-breaking folds become rare/absent (the budget ceiling rises to match so the budget gates don't front-evict first). `0` = keep turn-count folding. Benchmark the reuse/TTFT/quality/savings tradeoff. |
